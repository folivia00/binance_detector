...
### Stage 08: Live paper analysis completed

По итогам `live_paper_loop_report_20260324T171057Z.md` и stage-дока Stage 08 зафиксировано:

- `2167` total evaluations;
- `69` observed round ids;
- `67` оценочно полных 5m раундов;
- `2164/2167` live snapshots и `3` demo fallbacks;
- effective cadence ≈ `9.36s`, а не `5s`;
- `159` allowed entries;
- policy в основном разрешает вход (`1858 / 2167`);
- главный execution bottleneck: `spread_too_wide` и `slippage_too_high`;
- `very_strong` перенасыщен (≈ `67.88%`), что уже указывает на необходимость recalibration.

Также в рамках Stage 08 внесён критически важный фикс:
- demo fallback больше не участвует как нормальный signal tick в stateful live runner;
- такие события маркируются отдельно и не должны загрязнять round analytics.
...
- tier calibration пока перенасыщает `very_strong`, из-за чего tier analytics теряет селективность;
- signal quality ещё не доказана на реальных последовательных рыночных данных после recalibration;
- Binance layer уже лучше защищён от demo contamination, но long-run integrity всё равно нужно продолжать проверять;
...
7. **Stage 08 live paper reporting** ✅
8. **Следующий шаг: configurable tier calibration**
9. **Повторный live-paper прогон и markdown diff до/после calibration**
...
### Шаг 6
Сделать **config-driven tier calibration**, чтобы уменьшить saturation верхних tiers, прежде всего `very_strong`.

### Шаг 7
Повторить длинный `live_paper_loop` после recalibration и выпустить новый markdown-отчёт с прямым сравнением:
- tier distribution до/после;
- allowed entries до/после;
- block reasons до/после;
- execution bottlenecks до/после;
- time × distance usefulness до/после.
